...
export default function HostLobbyPage() {
  ...
  const playerMap = useMemo(
    () => new Map((lobbyState?.players ?? []).map((p) => [p.id, p.name])),
    [lobbyState?.players]
  );
  const {
    teamVoteReveal,
  ...
  useGameBackground(
    lobbyState?.gameState === "started" && Boolean(game)
  );

  if (!hostSecret) {
    return (
      <div className="flex min-h-screen items-center justify-center px-6">
        <Card variant="parchment" className="max-w-md space-y-3 text-center">
    ...
    <>
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
          ...
          <div className="flex items-center gap-4">
            <div className="relative h-14 w-14 shrink-0">
              <Image
                ...
                priority
                sizes="56px"
                className="object-contain drop-shadow-[0_0_18px_rgba(201,168,76,0.25)]"
              />
            </div>
        ...
        <Card variant="parchment" className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--parchment-dim)]">
                Share this link or let each knight scan the seal from their own
                device.
              </p>
            </div>
          </div>
          ...
          <div className="ornate-frame flex justify-center rounded-xl bg-[#07090d] px-4 py-6">
            {qrCode ? (
              <img
                src={qrCode}
        ...
        {hostPlayerName && (
          <Card className="flex flex-col gap-3">
            ...
            <p className="text-sm text-[var(--parchment-dim)]">
              Open your own player view to reveal your role once the quest
              begins — on this device or any other.
            </p>
            <Button
              variant="outline"
        ...
        {game && lobbyState?.players?.length ? (
          ...
          >
            {game.phase === "assassination" && (
              <p className="text-sm text-[var(--gold)] font-display tracking-wide">
